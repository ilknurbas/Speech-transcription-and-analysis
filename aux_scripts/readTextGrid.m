...
if LONGFT
    ...
    while i <= nd
        ...
        if strfind(data{1}{i},'name =')
            ...
            tmp = strfind(tier_name,'.'); % remove periods from tier names as they break struct pointers
            tier_name(tmp) = [];
            
            tmp = strfind(tier_name,'@');
            if(~isempty(tmp))
                tier_name = [tier_name(1:tmp-1) 'at' tier_name(tmp+1:end)];
            end
